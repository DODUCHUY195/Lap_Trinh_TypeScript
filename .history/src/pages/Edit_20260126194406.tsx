...
function EditSimple() {
  ...
  useEffect(() => {
    ...
    const getOne = async () => {
      try {
        ...
        const { data } = await axios.get<Subject>(`/api/subjects/${id}`, { headers });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
      await axios.put(`/api/subjects/${id}`, form, { headers });
  ...
  const reloadList = async () => {
    try {
      ...
      const token = localStorage.getItem("accessToken");
      const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
      await axios.get(`/api/subjects?${params.toString()}`, { headers });
      // Có thể dùng toast nếu muốn phản hồi
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <form className="space-y-6" onSubmit={onSubmit}>
        ...
        <button type="button" onClick={reloadList} className="ml-3 px-5 py-2 border rounded-lg">
          Tải danh sách
        </button>
      </form>
    </div>
