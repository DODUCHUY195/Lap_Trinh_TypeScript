// Trang List: Hiển thị danh sách môn học với tìm kiếm theo tên, lọc theo giáo viên và phân trang
// - Gọi API qua axios tới json-server/json-server-auth
// - name_like: tìm theo tên (không phân biệt hoa/thường)
// - _page/_limit: phân trang
// - Authorization: kèm token khi cần (PUT/POST/DELETE; GET tùy rules)
...
function ListSimple() {
  ...
  useEffect(() => {
    const getTeachers = async () => {
      try {
        // Đọc token (nếu đã login) để gắn Authorization cho API cần quyền
        const token = localStorage.getItem("accessToken");
        const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
        // Gọi API lấy toàn bộ (phân trang lớn) rồi suy ra danh sách giáo viên duy nhất
        // name_like không cần ở đây vì chỉ cần teacher
        const res = await axios.get<Subject[]>("/api/subject?_page=1&_limit=1000", { headers });
        const unique = Array.from(new Set(res.data.map((s) => s.teacher))).filter(Boolean);
        // Thêm "Tất cả" đứng đầu để tiện lọc
  ...
  useEffect(() => {
    const loadSubjects = async () => {
      try {
        ...
        // Xây dựng query: tìm theo name_like và lọc teacher
        ...
        // Phân trang theo _page/_limit (chuẩn json-server)
        ...
        // Đọc tổng bản ghi từ header x-total-count để tính tổng trang
    ...
    <div className="p-6">
        ...
        <button
          ...
          onClick={() => {
            ...
            const run = async () => {
              try {
                const params = new URLSearchParams();
                // Sử dụng cùng logic như loadSubjects để refetch thủ công
                if (searchTerm.trim()) params.set("name_like", searchTerm.trim());
                if (teacherFilter && teacherFilter !== "Tất cả") params.set("teacher", teacherFilter);
                params.set("_page", String(currentPage));
                params.set("_limit", String(itemsPerPage));
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const token = localStorage.getItem("accessToken");
                        const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
                        // Kèm token để đảm bảo quyền xóa (json-server-auth)
                        await axios.delete(`/api/subject/${item.id}`, { headers });
                        // Xóa khỏi state để cập nhật UI
                        // Cập nhật state để UI loại bỏ dòng đã xóa ngay
      ...
      <div className="flex items-center gap-2 mt-4">
        <button
          ...
          // Previous: lùi về trang trước, không nhỏ hơn 1
          disabled={page === 1}
        ...
        {/* Render danh sách số trang, tô màu trang hiện tại */}
        {pages.map((p) => (
          <button
            key={p}
        ...
        <button
          ...
          // Next: tiến về trang sau, không vượt quá tổng trang
